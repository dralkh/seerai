...
export class OcrService {
  ...
  constructor() {
    this.apiKey = Zotero.Prefs.get(
      `${config.prefsPrefix}.datalabApiKey`,
    ) as string;
  }
  ...
  private buildMultipartBody(
    ...
    const body = new Uint8Array(
      preFileBytes.length + fileData.length + postFileBytes.length,
    );
  ...
  public async convertToMarkdown(
    ...
    ztoolkit.log(
      `OCR: Raw mode preference value: "${rawMode}" (type: ${typeof rawMode})`,
    );
    ...
    if (!mode) {
      ...
      const useLocal = Zotero.Prefs.get(
        `${config.prefsPrefix}.datalabUseLocal`,
      ) as boolean;
      mode = useLocal ? "local" : "cloud";
      ztoolkit.log(
        `OCR: Using fallback mode: "${mode}" (useLocal: ${useLocal})`,
      );
    }
    ...
    const mistralApiKey = Zotero.Prefs.get(
      `${config.prefsPrefix}.mistralApiKey`,
    ) as string;

    ztoolkit.log(
      `OCR: API Keys - DataLab: ${this.apiKey ? "set" : "NOT SET"}, Mistral: ${mistralApiKey ? "set" : "NOT SET"}`,
    );

    // Validate API keys based on mode
    if (mode === "cloud" && !this.apiKey) {
      const msg =
        "Error: DataLab API Key is missing. Please set it in preferences.";
      ztoolkit.log(msg);
      if (showProgress) {
    ...
    if (mode === "mistral" && !mistralApiKey) {
      const msg =
        "Error: Mistral API Key is missing. Please set it in preferences.";
      ztoolkit.log(msg);
      if (showProgress) {
        new ztoolkit.ProgressWindow("Mistral OCR")
          .createLine({
            text: msg,
            icon: "warning",
            progress: 100,
    ...
    if (!isPdfAttachment) {
      ...
      ztoolkit.log(msg);
      if (showProgress) {
        new ztoolkit.ProgressWindow("OCR Error")
          .createLine({
            text: msg,
            icon: "warning",
            progress: 100,
    ...
    try {
      ...
      try {
        ...
        ztoolkit.log(
          `DataLab: getFilePathAsync failed: ${(err as Error).message}`,
        );
      }
      ...
      if (
        ...
        if (dataDirPath) {
          ...
          ztoolkit.log(
            `DataLab: Manual parts - Key: ${itemKey}, FileName: ${fileName}`,
          );
          ...
          ztoolkit.log(
            "DataLab: Error - Zotero.DataDirectory.dir is undefined!",
          );
        }
      }
      ...
      if (mode === "local") {
        ...
        let localApiUrl =
          (Zotero.Prefs.get(`${config.prefsPrefix}.datalabUrl`) as string) ||
          "http://localhost:8001";
        // Normalize URL: remove trailing slash and common API suffixes that might be pasted by mistake
        ...
        const localForceOcr =
          (Zotero.Prefs.get(
            `${config.prefsPrefix}.localForceOcr`,
          ) as boolean) ?? true;
        ...
        ztoolkit.log(
          `DataLab: Posting to ${localApiUrl}/marker with filepath: ${filePath}`,
        );
        ...
        if (!markdown) {
          throw new Error(
            "Could not extract markdown from local server response. Check debug logs for response structure.",
          );
        }
        ...
        const urlResponse = await Zotero.HTTP.request(
          "GET",
        ...
        const ocrResponse = await Zotero.HTTP.request(
          "POST",
        ...
        if (!markdownContent) {
          throw new Error(
            "Could not extract markdown from Mistral OCR response",
          );
        }
        ...
        try {
          ...
          ztoolkit.log(
            "Mistral: Failed to delete file (non-critical)",
            deleteErr,
          );
        }
        ...
        const cloudForceOcr =
          (Zotero.Prefs.get(
            `${config.prefsPrefix}.cloudForceOcr`,
          ) as boolean) ?? false;
        const cloudUseLlm =
          (Zotero.Prefs.get(`${config.prefsPrefix}.cloudUseLlm`) as boolean) ??
          false;
        ...
        const uploadResult =
          uploadResponse.response as unknown as DatalabResponse;
        ...
        const result = await this.pollForResults(
          uploadResult.request_check_url,
          progressLine,
        );
  ...
  private async saveNote(attachmentItem: Zotero.Item, markdown: string) {
    ...
    if (attachmentItem.parentID) {
      ...
      ztoolkit.log(
        `DataLab: Attaching note to parent item ${attachmentItem.parentID}`,
      );
    } else {
      note.parentID = attachmentItem.id;
      ztoolkit.log(
        `DataLab: PDF is top-level, attaching note to PDF ${attachmentItem.id}`,
      );
    }
    ...
    note.setNote(
      `<h1>${this.escapeHtml(title as string)}</h1><pre style="white-space: pre-wrap;">${this.escapeHtml(markdown)}</pre>`,
    );
    await note.saveTx();
  }
