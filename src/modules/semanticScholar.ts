...
class SemanticScholarService {
  ...
  private readonly recommendationsUrl =
  ...
  private buildQueryParams(options: SearchOptions): URLSearchParams {
    ...
    if (
      options.minCitationCount !== undefined &&
      options.minCitationCount > 0
    ) {
  ...
  async getRecommendations(
    ...
    Zotero.debug(
      `[seerai] Semantic Scholar recommendations for ${positivePaperIds.length} papers`,
    );
  ...
  async autocomplete(
    query: string,
  ): Promise<{ paperId: string; title: string }[]> {
    ...
    try {
      ...
      return (
        data.matches?.map((m) => ({ paperId: m.id, title: m.title })) || []
      );
    } catch (error) {
