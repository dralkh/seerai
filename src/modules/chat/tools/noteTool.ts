...
export async function executeNote(
  ...
  switch (params.action) {
    ...
    default:
      return {
        success: false,
        error: `Unknown note action: ${(params as any).action}`,
      };
  }
}
...
async function executeCreateNote(
  ...
  try {
    ...
    Zotero.debug(
      `[seerai] Tool: create_note parent=${parent_item_id} col=${collection_id} title="${title}"`,
    );
    ...
    if (parent_item_id) {
      ...
      if (parentItem && !Assistant.checkItemInScope(parentItem, config)) {
        return {
          success: false,
          error: `Permission Denied: Parent item ${parent_item_id} is outside the current restricted scope.`,
        };
      }
    }
    ...
    if (
      !lowerHtml.includes(`<h1>${lowerTitle}`) &&
      !lowerHtml.includes(`<h2>${lowerTitle}`) &&
      !lowerHtml.includes(`<strong>${lowerTitle}`)
    ) {
...
async function executeEditNote(
  ...
  try {
    ...
    Zotero.debug(
      `[seerai] Tool: edit_note id=${note_id} ops=${operations.length}`,
    );
    ...
    if (!item.isNote()) {
      ...
      if (item.isRegularItem()) {
        ...
        if (noteIds && noteIds.length > 0) {
          ...
          Zotero.debug(
            `[seerai] Tool: edit_note resolved parent item ${note_id} to child note ${actualNoteId}`,
          );

          if (noteIds.length > 1) {
            Zotero.debug(
              `[seerai] Tool: edit_note warning: item ${note_id} has ${noteIds.length} child notes, using first one (${actualNoteId})`,
            );
          }
        } else {
          return {
    ...
    for (const op of operations) {
      ...
      if (result.success) {
        ...
        Zotero.debug(
          `[seerai] Tool: edit_note op '${op.type}' failed: ${result.error}`,
        );
    ...
    if (operationsApplied === 0) {
      return {
        success: false,
        error:
          "No operations were successfully applied. Check that search terms exist in the note.",
      };
    }
    ...
    Zotero.debug(
      `[seerai] Tool: edit_note completed ${operationsApplied}/${operations.length} ops`,
    );
    ...
    const resolvedMsg =
      actualNoteId !== note_id ? ` (resolved from parent item ${note_id})` : "";
  ...
  switch (op.type) {
    case "replace": {
      if (!op.search) {
        return {
          success: false,
          error: "'replace' operation requires 'search' parameter",
        };
      }
      if (op.content === undefined) {
        return {
          success: false,
          error: "'replace' operation requires 'content' parameter",
        };
      }
      ...
      if (!content.includes(op.search)) {
        return {
          success: false,
          error: `Search text not found: "${op.search.substring(0, 50)}..."`,
        };
      }
    ...
    case "delete": {
      if (!op.search) {
        return {
          success: false,
          error: "'delete' operation requires 'search' parameter",
        };
      }
    ...
    default:
      return {
        success: false,
        error: `Unknown operation type: ${(op as EditNoteOperation).type}`,
      };
  }
}
