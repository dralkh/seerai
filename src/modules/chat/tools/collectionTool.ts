...
export async function executeCollection(
  ...
  switch (params.action) {
    case "find":
      return executeFindCollection(
        {
          name: params.name!,
          library_id: params.library_id,
          parent_collection_id: params.parent_id,
        },
        config,
      );
    case "create":
    ...
    case "add_item":
      ...
      if (!params.item_ids || params.item_ids.length === 0) {
        return {
          success: false,
          error: "item_ids required for add_item action",
        };
      }
      let added = 0;
      for (const item_id of params.item_ids) {
        const result = await executeMoveItem(
          {
            item_id,
            target_collection_id: params.collection_id!,
    ...
    case "remove_item":
      if (!params.item_ids || params.item_ids.length === 0) {
        return {
          success: false,
          error: "item_ids required for remove_item action",
        };
      }
      let removed = 0;
      for (const item_id of params.item_ids) {
        const result = await executeRemoveItemFromCollection(
          { item_id, collection_id: params.collection_id! },
          config,
        );
    ...
    default:
      return {
        success: false,
        error: `Unknown collection action: ${(params as any).action}`,
      };
  }
}
...
async function executeFindCollection(
  ...
  try {
    ...
    Zotero.debug(
      `[seerai] Tool: find_collection name="${name}" lib=${library_id} parent=${parent_collection_id}`,
    );
    ...
    if (parent_collection_id) {
      ...
      if (parent) {
        ...
        if (config.libraryScope.type === "collection") {
          ...
          if (
            parent_collection_id !== scopedId &&
            !isCollectionChildOf(parent, scopedId)
          ) {
            Zotero.debug(
              `[seerai] Tool: find_collection - parent ${parent_collection_id} is outside scoped collection ${scopedId}`,
            );
            return {
              success: true,
              data: { collections: [] },
              summary: `Search restricted to scoped collection subtree.`,
            };
          }
        }
        // Get sub-collections
      ...
      if (
        ...
        config.libraryScope.type === "group" &&
        library_id !==
          Zotero.Groups.getLibraryIDFromGroupID(config.libraryScope.groupId)
      ) {
        collections = [];
      } else {
      ...
      if (config.libraryScope.type === "user") {
        ...
        const libId = Zotero.Groups.getLibraryIDFromGroupID(
  ...
  try {
    ...
    if (parent_collection_id) {
      ...
      if (!parent) {
        return {
          success: false,
          error: `Parent collection ${parent_collection_id} not found`,
        };
      }
      // Check existing children
      const children = parent.getChildCollections();
      existingCollection = children.find(
      ...
      existingCollection = allCollections.find(
        (c) => !c.parentID && c.name.toLowerCase() === searchName,
      );
    ...
    if (existingCollection) {
      Zotero.debug(
        `[seerai] Tool: create_collection found existing "${existingCollection.name}" (ID: ${existingCollection.id})`,
      );
      return {
        success: true,
        data: {
...
async function executeListCollection(
  ...
  try {
    ...
    if (config.libraryScope.type === "collection") {
      const scopedId = (config.libraryScope as any).collectionId;
      if (
        collection_id !== scopedId &&
        !isCollectionChildOf(collection, scopedId)
      ) {
        return {
          success: false,
      ...
      return {
        success: false,
        error: `Permission Denied: Collection is in a different library.`,
      };
    } else if (config.libraryScope.type === "group") {
...
async function executeMoveItem(
  ...
  try {
    ...
    Zotero.debug(
      `[seerai] Tool: move_item item=${item_id} target=${target_collection_id}`,
    );
    ...
    const wasAlreadyInTarget =
      currentCollections.includes(target_collection_id);
    ...
    Zotero.debug(
      `[seerai] Tool: move_item - Item "${item.getField("title")}" is currently in collections: [${currentCollectionNames.join(", ")}]`,
    );
    ...
    if (remove_from_others && currentCollections.length > 1) {
      const removedCount = wasAlreadyInTarget
...
async function executeRemoveItemFromCollection(
  ...
  try {
    ...
    Zotero.debug(
      `[seerai] Tool: remove_item_from_collection item=${item_id} col=${collection_id}`,
    );
