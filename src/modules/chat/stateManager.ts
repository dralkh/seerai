...
export class ChatStateManager {
  ...
  // Add a selection
  addSelection<T extends StateName>(
    type: T,
    selection: ChatStates[T][number],
  ): boolean {
  ...
  addSelections<T extends StateName>(
    type: T,
    selections: ChatStates[T],
  ): number {
    let added = 0;
  ...
  // Toggle selection
  toggleSelection<T extends StateName>(
    type: T,
    selection: ChatStates[T][number],
  ): boolean {
  ...
  async compileContext(): Promise<string> {
    ...
    if (this.states.collections.length > 0) {
      ...
      for (const collection of this.states.collections) {
        contextParts.push(
          `- ${collection.title}${collection.itemCount ? ` (${collection.itemCount} items)` : ""}`,
        );
      }
    }
  ...
  /**
   * Get estimated token count for the current compiled context
   */
  async getContextTokenCount(items: any[]): Promise<number> {
    // This is a placeholder - actual expansion happens in Assistant.ts
    // But we provide a baseline count here
    let total = 0;
  ...
  /**
   * Get total session tokens (accumulated across all messages)
   */
  getSessionCumulativeTokens(messages: any[]): number {
    let total = 0;
    for (const msg of messages) {
      total += ChatStateManager.countTokens(msg.content);
      // Also count tool results if present
      if (msg.toolResults) {
        total += ChatStateManager.countTokens(JSON.stringify(msg.toolResults));
      }
    }
    return total;
  }
