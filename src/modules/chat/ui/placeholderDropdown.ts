...
export function initPlaceholderAutocomplete(
  ...
  Zotero.debug(
    `[seerai] initPlaceholderAutocomplete: Initializing on input element`,
  );

  const handleInput = (e?: Event) => {
    Zotero.debug(
      `[seerai] handleInput: fired, value="${input.value}", cursorPos=${input.selectionStart}`,
    );
    ...
    if (trigger) {
      Zotero.debug(
        `[seerai] handleInput: Trigger found: ${trigger.trigger} type=${trigger.type} query="${trigger.query}"`,
      );
...
async function showPlaceholderDropdown(
  ...
  Zotero.debug(
    `[seerai] Placeholder triggered: ${trigger.trigger} with query "${trigger.query}"`,
  );
  ...
  try {
    ...
    Zotero.debug(
      `[seerai] Got ${state.results.length} results for ${trigger.type}`,
    );
  } catch (error) {
  ...
  Zotero.debug(
    `[seerai] Dropdown parent: ${state.dropdown.parentElement?.tagName || "none"}`,
  );
...
function selectCurrentItem(
  ...
  if (selectedResult.type === "prompt" && selectedResult.data?.template) {
    ...
    input.value = insertPlaceholderValue(
      input.value,
      state.trigger,
      valueToInsert,
      itemId,
    ...
    const displayLength =
      Math.min(valueToInsert.length, maxDisplayLength - 3) +
      (valueToInsert.length > maxDisplayLength ? 3 : 0);
    const idLength = itemId ? String(itemId).length + 2 : 0; // +2 for ::
    const newCursorPos =
      state.trigger.startPos + 1 + 1 + displayLength + idLength + 1 + 1; // [ + trigger + display + ::id + ] + space
...
function positionDropdown(input: HTMLInputElement): void {
  ...
  Zotero.debug(
    `[seerai] positionDropdown: dropdown parent=${state.dropdown.parentElement?.tagName || "none"}`,
  );
}
 ...
 */
export function triggerNextPlaceholder(
  doc: Document,
  input: HTMLInputElement,
): void {
  ...
  if (foundMatch) {
    Zotero.debug(
      `[seerai] triggerNextPlaceholder: Found ${foundMatch.text} at ${foundMatch.end}`,
    );
